...
export default function BusinessChatPage() {
  ...
  const { data: conversations, isLoading: loadingConvs } = useQuery({
    queryKey: ["business-conversations", user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      ...
      <div className={cn(
        ...
        {selectedConv ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-[#f0f2f5] dark:bg-[#222e35]">
            ...
            <h2 className="text-lg font-bold text-foreground">Nexus Chat</h2>
            <p className="text-xs text-muted-foreground max-w-xs text-center px-4">
              Selecione uma conversa para começar a falar com o suporte ou gerenciar seus pedidos.
            </p>
          </div>
        )}
      </div>
