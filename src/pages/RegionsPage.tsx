...
export default function RegionsPage() {
  ...
  useEffect(() => {
    ...
    // Collect all coords from all region geometries
  ...
  // Only auto-center once when regions are first loaded
  ...
  const selectCity = (item: any) => {
    ...
    if (m && item.geometry?.coordinates) {
      m.flyTo({ center: item.geometry.coordinates as [number, number], zoom: 11, duration: 1500 });
    }
  ...
  return (
    <AdminLayout title="Regiões" subtitle="Gestão de regiões e precificação" fullHeight>
      <div className="flex flex-col lg:flex-row h-full w-full relative min-w-0 min-h-0 overflow-hidden">
        {/* Map */}
