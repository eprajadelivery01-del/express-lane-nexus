...
function NewDeliveryForm({ onClose, userId }: { onClose: () => void; userId?: string }) {
  ...
  // Geocode address and find region
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("deliveries").insert({
      ...
      region_id: null, // Will be set by backend if needed
      notes: notes || null,
    });
  ...
  return (
    <div className="space-y-4">
      ...
      <h2 className="text-lg font-display font-bold text-foreground">Nova Entrega</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="text-sm font-medium mb-1.5 block">Endereço de entrega</label>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-2.5 rounded-xl bg-primary/10 text-primary text-sm font-medium hover:bg-primary/20 disabled:opacity-50 shrink-0"
            >
        ...
        {/* Region & Price result */}
        {regionInfo && (
          <div className="bg-card rounded-xl border-2 p-4 shadow-card" style={{ borderColor: regionInfo.color }}>
        ...
        {coords && !regionInfo && !searching && (
          <div className="bg-warning/5 border border-warning/20 rounded-xl p-3 text-xs text-warning">
