...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      console.log(`[LoginPage - Admin] Verificando acesso para user.id: ${user.id}, roles:`, roles);
      const timer = setTimeout(() => {
        if (hasRole("admin")) {
          console.log("[LoginPage - Admin] Role ADMIN detectada. Navegando para /admin");
          navigate("/admin");
        } else {
          console.warn("[Admin - bfeb2f00] Acesso negado: Perfil não administrativo.");
