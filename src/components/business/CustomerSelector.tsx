...
import { Search, User, Phone, MapPin, Plus, Loader2, ArrowLeft, Pencil } from "lucide-react";
...
function parseAddressLabel(fullStr: string): { label: string; address: string } {
  if (fullStr.startsWith("[")) {
    const idx = fullStr.indexOf("]");
    if (idx !== -1) {
      return {
        label: fullStr.slice(1, idx),
        address: fullStr.slice(idx + 1).trim()
      };
    }
  }
  return { label: "Casa", address: fullStr };
}
