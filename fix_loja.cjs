...
async function run() {  
  const { error } = await supabase.from('companies').update({ name: 'Loja De Teste', active: true, is_active: true, is_open: true, show_in_marketplace: true }).eq('id', 'd96dcc95-f348-4975-90ce-fd472d7a0d81');  
  console.log('Update result (using standard anon key):', error);  
}  
run();  
